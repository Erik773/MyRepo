...
Функция ОписаниеСовпадающихМетодов(Метод, СледующийМетод)
	...
	СсылкаПереходаКБлокуОписанияСервер = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("<a href=""%1"">%2</a> " + НРег(СтрокаДляВызоваССервера()), "#_" + СерверныйМетод.ДоступностьИдентификатор + АдресСсылки(СерверныйМетод), ПолноеИмяМетодаПоИмениМодуля(СерверныйМетод));
	СсылкаПереходаКБлокуОписанияКлиент  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("<a href=""%1"">%2</a> " + НРег(СтрокаДляВызоваСКлиента()), "#_" + КлиентскийМетод.ДоступностьИдентификатор + АдресСсылки(КлиентскийМетод), ПолноеИмяМетодаПоИмениМодуля(КлиентскийМетод));
...
Функция СтрокаПредусмотреноДвеФункции()
	...
	Если АнглийскаяВерсия Тогда
		Возврат "There are two functions with the same name";
	Иначе
		Возврат "Предусмотрено две одноименных функции";
	КонецЕсли;
...
Функция СтрокаКлиентИСервер()
	...
	Если АнглийскаяВерсия Тогда
		Возврат "ClientAndServer";
	Иначе
		Возврат "КлиентИСервер";
	КонецЕсли;
...
Функция СтрокаОбщийМодуль()
	...
	Если АнглийскаяВерсия Тогда
		Возврат "Common module";
	Иначе
		Возврат "Общий модуль";
	КонецЕсли;
...
Функция СтрокаДляВызоваССервера()
	...
	Если АнглийскаяВерсия Тогда
		Возврат "To call from the server";
	Иначе
		Возврат "Для вызова с сервера";
	КонецЕсли;
...
Функция СтрокаДляВызоваСКлиента()
	...
	Если АнглийскаяВерсия Тогда
		Возврат "To call from the client";
	Иначе
		Возврат "Для вызова с клиента";
	КонецЕсли;
...
Функция СтрокаДляВызоваСКлиентаИСервера()
	...
	Если АнглийскаяВерсия Тогда
		Возврат "To call both from the client and server";
	Иначе
		Возврат "Для вызова с клиента и сервера";
	КонецЕсли;
...
Функция СтрокаПримерРеализации()
	...
	Если АнглийскаяВерсия Тогда
		Возврат "Implementation example";
	Иначе
		Возврат "Пример реализации";
	КонецЕсли;
...
Функция СтрокаПримерВызова()
	...
	Если АнглийскаяВерсия Тогда
		Возврат "Call example";
	Иначе
		Возврат "Пример вызова";
	КонецЕсли;
...
Функция СтрокаСсылкаНаСтандарты()
	...
	Если АнглийскаяВерсия Тогда
		Возврат "<a href=""http://its.1c.ru/db/v8std"">«Системе стандартов AND методик разработки конфигураций для платформы 1С:Предприятие 8»</a>";
	Иначе
		Возврат "<a href=""http://its.1c.ru/db/v8std"">«Системе стандартов и методик разработки конфигураций для платформы 1С:Предприятие 8»</a>";
	КонецЕсли;
...
Функция СтрокаПодробности()
	...
	Если АнглийскаяВерсия Тогда
		...
		+ "<p class=""Paragraph0c"">" + "See more about application interface and overridable modules in 
			|%1,
			|the ""Library development and usage section""." + "</p>";
	Иначе
		Подробности = "<p class=""Paragraph0c"">"
...
Функция СтрокаГлава4ПрограммныйИнтерфейс()
	...
	Если АнглийскаяВерсия Тогда
		Возврат "Chapter 4. Application interface";
	Иначе
		Возврат "Глава 4. Программный интерфейс";
	КонецЕсли;
...
Функция СтрокаТолькоДляВнутреннегоИспользования()
	...
	Если АнглийскаяВерсия Тогда
		Возврат "For internal use only";
	Иначе
		Возврат "Только для внутреннего использования";
	КонецЕсли;
...
Функция СтрокаОбластьДляВызоваИзДругихПодсистем()
	...
	Если АнглийскаяВерсия Тогда
		Возврат "#Region ForCallsFromOtherSubsystems";
	Иначе
		Возврат "#Область ДляВызоваИзДругихПодсистем";
	КонецЕсли;
...
Функция СтрокаОбластьУстаревшиеПроцедурыИФункции()
	...
	Если АнглийскаяВерсия Тогда
		Возврат "#Region ObsoleteProceduresAndFunctions";
	Иначе
		Возврат "#Область УстаревшиеПроцедурыИФункции";
	КонецЕсли;
