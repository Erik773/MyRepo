...
Процедура ДобавитьФормулу(Команда)
	...
	ПараметрыРедактированияФормулы.Операторы = Операторы;
...
Процедура ПослеИзмененияФормулы(ОписаниеФормулы, Формула) Экспорт 
	...
	Строка = Элементы.ДоступныеПоля.ТекущиеДанные;
...
Процедура УстановитьИмяКоллекции()
	...
	Режимы.Вставить("УсловноеОформление", "УсловноеОформление.ДоступныеПоляПолей");
...
Процедура ИнициализироватьСписокДоступныхПолей()
	...
	Операторы = ПоместитьВоВременноеХранилище(КонструкторФормул.СписокОператоров("ВсеОператорыСКД"), УникальныйИдентификатор);
